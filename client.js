...
function readyNow(){
  $('#addSalary').on('click', displayBonus);
}

function displayBonus() {
  newEmployeeObject(employees);
  let bonusElement = $(bonusOut);
  bonusElement.empty();
  for (employee of employees){
    bonusElement.append(employee.name + ` ` +employee.bonusPercentage + ` ` + employee.totalCompensation + ` ` + employee.totalBonus);
  }
}
